...
export default function QuizSection() {
  ...
  const handleDeleteQuestion = async (id) => {
    ...
    try {
      const response = await fetch(
        `${API_BASE_URL}/quizzquestion/${id}/delete`,
        {
          method: "DELETE",
        }
      );
  ...
  const handleUpdateQuestion = async (e) => {
    ...
    try {
      const response = await fetch(
        `${API_BASE_URL}/quizzquestion/${editingQuestion.id}/edit`,
        {
          method: "PUT",
          headers: {
  ...
  const handleAddQuestion = async (e) => {
    ...
    try {
      const response = await fetch(`${API_BASE_URL}/quizzquestion/create`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        },
          ...
          {quizQuestions.length === 0 ? (
            <div className="text-center py-12">
              <div className="text-gray-500 text-lg mb-4">
                {loading ? "Chargement..." : "Aucune question de quiz trouvée"}
              </div>
              {!loading && (
                <button
                  onClick={() => setShowAddQuestion(true)}
              ...
              <div className="space-y-4">
                ...
                <div>
                  ...
                  >
                    ...
                    {newQuestion.choiceC && (
                      <option value={2}>Option C: {newQuestion.choiceC}</option>
                    )}
                    {newQuestion.choiceD && (
                      <option value={3}>Option D: {newQuestion.choiceD}</option>
                    )}
                  </select>
                </div>
